...
angular.module('frapontillo.bootstrap-switch', []).directive('bsSwitch', [
  ...
  function ($timeout) {
    return {
      template: '<div class="make-switch" data-on-label="{{switchOnLabel}}" data-off-label="{{switchOffLabel}}" ' + 'data-text-label="{{switchLabel}}" data-icon-label="{{switchIcon}}" ' + 'data-animated="{{switchAnimate}}" ng-class="{{getSizeClass()}}">' + '  <input type="checkbox" ng-model="ngModel"/>' + '</div>',
      restrict: 'EA',
      replace: true,
      transclude: true,
      scope: {
